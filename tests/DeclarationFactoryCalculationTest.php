...
use Carbon\Carbon;
use Mijnkantoor\Belastingdienst\DeclarationFactory;
use Mijnkantoor\Belastingdienst\Enums\BlockTypes;
use Mijnkantoor\Belastingdienst\Enums\DeclarationTypes;
use PHPUnit\Framework\TestCase;

class DeclarationFactoryCalculationTest extends TestCase
{
    public function testBlockCalculations(): void
    {
        $factory = new DeclarationFactory();
    ...
    public function testPeriodCalculations(): void
    {
        ...
        $this->assertEquals(
            1,
            $factory->calculatePeriod(BlockTypes::MONTHLY, new Carbon('01-01-2020'))
        );
        $this->assertEquals(
            2,
            $factory->calculatePeriod(BlockTypes::MONTHLY, new Carbon('01-02-2020'))
        );
        $this->assertEquals(
            3,
            $factory->calculatePeriod(BlockTypes::MONTHLY, new Carbon('01-03-2020'))
        );
        $this->assertEquals(
            4,
            $factory->calculatePeriod(BlockTypes::MONTHLY, new Carbon('01-04-2020'))
        );
        $this->assertEquals(
            5,
            $factory->calculatePeriod(BlockTypes::MONTHLY, new Carbon('01-05-2020'))
        );
        $this->assertEquals(
            6,
            $factory->calculatePeriod(BlockTypes::MONTHLY, new Carbon('01-06-2020'))
        );

        //Four weeks
        $this->assertEquals(
            1,
            $factory->calculatePeriod(BlockTypes::FOURWEEK, new Carbon('01-01-2020'))
        );

        $this->assertEquals(
            2,
            $factory->calculatePeriod(BlockTypes::FOURWEEK, new Carbon('27-01-2020'))
        );

        $this->assertEquals(
            3,
            $factory->calculatePeriod(BlockTypes::FOURWEEK, new Carbon('24-02-2020'))
        );

        $this->assertEquals(
            13,
            $factory->calculatePeriod(BlockTypes::FOURWEEK, new Carbon('30-11-2020'))
        );

        $this->assertEquals(
            1,
            $factory->calculatePeriod(BlockTypes::FOURWEEK, new Carbon('01-01-2021'))
        );

        //Half year
        $this->assertEquals(
            1,
            $factory->calculatePeriod(BlockTypes::HALFYEAR, new Carbon('01-01-2020'))
        );
        ...
        $this->assertEquals(
            1,
            $factory->calculatePeriod(BlockTypes::QUARTER, new Carbon('01-01-2020'))
        );

        // Shifted Quarter
        $this->assertEquals(
            1,
            $factory->calculatePeriod(BlockTypes::QUARTER, new Carbon('01-02-2020'))
        );
        $this->assertEquals(
            1,
            $factory->calculatePeriod(BlockTypes::QUARTER, new Carbon('01-03-2020'))
        );
        $this->assertEquals(
            2,
            $factory->calculatePeriod(BlockTypes::QUARTER, new Carbon('01-04-2020'))
        );
        ...
        $this->assertEquals(
            0,
            $factory->calculatePeriod(BlockTypes::YEARLY, new Carbon('01-01-2020'))
        );
    ...
    public function testLoanNumber(): void
    {
        ...
        $periods = [
            ...
            DeclarationTypes::REVENUE->value => [
                // month
        ...
        foreach ($periods as $declarationType => $testRules) {
            foreach ($testRules as $testRule) {

                $declaration = $factory->createFromDeclarationIdAndDateRange(
                    DeclarationTypes::tryFrom($declarationType),
                    $testRule[2],
